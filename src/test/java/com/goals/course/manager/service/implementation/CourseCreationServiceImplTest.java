...
class CourseCreationServiceImplTest {
    ...
    @InjectMocks
    private CourseCreationServiceImpl service;
    ...
    void createCourse_checkResult() {
        ...
        when(mockCourseMapper.mapToCourseDTO(any())).thenReturn(mappedSavedCourse);
        when(mockCourseCreationValidation.validateCreation(any())).thenReturn(ValidationResult.valid());
        when(mockCourseRepository.save(any())).thenReturn(new Course());

        // WHEN
        final var result = service.createCourse(buildCourse());

        // THEN
        assertThat(result).isSameAs(mappedSavedCourse);
    }
